    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 animate-fade-in">
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl relative overflow-hidden">
        ...
        <div className="bg-gray-700 p-6 text-white">
          ...
          {product.brand && (
            <p className="text-blue-100 text-sm">Marca: {product.brand}</p>
          )}
        </div>
        ...
        <div className="p-6">
          ...
          <div className="mb-6">
            <div className="flex justify-between items-center mb-2">
              <p className="text-gray-600 text-sm">SKU: <span className="font-medium">{product.sku}</span></p>
              <p className="text-2xl font-bold text-green-600">S/. {product.price?.toFixed(2) || "N/A"}</p>
            </div>
            {product.description && (
              <p className="text-gray-700 text-sm leading-relaxed">{product.description}</p>
            )}
            <p className="text-gray-500 text-xs mt-2">Stock total: {product.total_stock} unidades</p>
          </div>

          {/* Selección de tallas */}
          <div className="mb-6">
            <h3 className="font-semibold text-lg mb-3 text-gray-800">Selecciona una talla</h3>
                  ...
                  >
                    ...
                    {v.stock === 0 && (
                      <div className="absolute inset-0 bg-red-500 bg-opacity-20 rounded-xl flex items-center justify-center">
                        <span className="text-red-600 font-bold text-xs">AGOTADO</span>
                      </div>
                    )}
                  </button>
                ))
              ) : (
                <p className="col-span-4 text-gray-500 text-center py-4">No hay tallas disponibles.</p>
              )}
            </div>
          </div>
          ...
          {selectedVariant && (
            <div className="mb-6">
              <h3 className="font-semibold text-lg mb-3 text-gray-800">Cantidad</h3>
              <div className="flex items-center justify-center gap-4">
                <button
                  onClick={() => handleQtyChange(-1)}
                  disabled={qty <= 1}
                  className="w-12 h-12 bg-gray-200 rounded-full flex items-center justify-center hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed transition"
                >
