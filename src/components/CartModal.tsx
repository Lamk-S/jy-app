...
export default function CartModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 animate-fade-in">
      <div className="bg-white p-6 rounded-2xl shadow-2xl max-w-md w-full mx-4 max-h-[80vh] overflow-y-auto relative">
        {/* Botón de cerrar */}
        <button
          onClick={onClose}
        ...
        {items.length === 0 ? (
          ...
          <ul className="space-y-4 mb-6">
            {items.map((item) => (
              <li key={`${item.productId}-${item.size}`} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg shadow-sm">
                <div className="flex-1">
                  <h3 className="font-semibold text-gray-800">{item.productName}</h3>
                  <p className="text-sm text-gray-600">Talla: {item.size || "N/A"} | Stock: {item.stock}</p>
                  <p className="text-sm text-gray-600">Precio: S/.{item.price?.toFixed(2) || "N/A"}</p>
                  <p className="text-sm font-medium text-gray-800">Subtotal: S/.{(item.price ? item.price * item.qty : 0).toFixed(2)}</p>
                </div>
